...
#path.Cal<-paste0('Data/',folder.date)
#file.names.Log<-basename(list.files(path.Log, pattern = c("CT","csv$"), recursive = F)) #list all csv file names in the folder and subfolders
...
# Filter out dates
condCal<-CT.data%>%filter(between(date,startCal1,endCal1)|between(date,startCal2,endCal2)) 
condLog<-CT.data%>%filter(between(date,Launch,Retrieval)) 
