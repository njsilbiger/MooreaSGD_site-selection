# Cleaned Respo Code for big plate community P/R #####
# Created by Nyssa Silbiger
# Edited on 7/22/2021
# Edited by Hendrikje Jorissen on 11/11/21
# Same as CalculatePR.R but adjusted for big plates analysis
...
path.p<-here("Data","August2022","PlateCommunityRespoData","RawO2Files") #the location of all your respirometry files

# bring in all of the individual files
filenames<-basename(list.files(path = path.p, pattern = "csv$", recursive = TRUE)) #list all csv file names in the folder and subfolders
...
Respo.R<- data.frame(matrix(NA, nrow=length(filenames), ncol=4))
...
for (i in 1: length(filenames)) {
  FRow<-which(Sample.Info$FileName==filenames[i]) #stringsplit this renames our file
  Respo.Data1 <-read_csv(file.path(path.p, paste0(filenames[i])), skip = 1) %>%
  ...
  pdf(paste0(here("Output","August2022","RespoOutput"),"/", rename,"thinning.pdf"))
...
write_csv(Respo.R, here("Data","August2022","PlateCommunityRespoData","Respo.R.csv"))  

Respo.R <- read_csv(here("Data","August2022","PlateCommunityRespoData","Respo.R.csv"))

# Calculate Respiration rate

Respo.R<-Respo.R %>% #PROBLEM Sample.Info is lost when joining => Maybe because of file names with extra O2??
